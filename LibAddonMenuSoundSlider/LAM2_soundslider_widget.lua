...
local function raiseSoundChangedCallback(panel, control, value)
    if control.data.playSound and value ~= "NONE" and soundsRef[value] ~= nil then
        PlaySound(soundsRef[value])
    end
    cm:FireCallbacks("LibAddonMenuSoundSlider_UpdateValue", panel or LAM.currentAddonPanel, control, value)
end
